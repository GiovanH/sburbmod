...
public class ContainerCruxtruder extends Container {
	...
	private TileCruxtruder tileCruxtruder;
	...
	public ContainerCruxtruder(InventoryPlayer invPlayer, TileCruxtruder tileCruxtruder) {
		this.tileCruxtruder = tileCruxtruder;
		...
		for (int y = 0; y < OUTPUT_SLOTS_COUNT; y++) {
			...
			addSlotToContainer(new SlotOutput(tileCruxtruder, slotNumber, OUTPUT_SLOTS_XPOS,
	...
	public boolean canInteractWith(EntityPlayer player) {
		return tileCruxtruder.isUsableByPlayer(player);
	}
	...
	public void detectAndSendChanges() {
		...
		boolean fieldHasChanged[] = new boolean[tileCruxtruder.getFieldCount()];
		if (cachedFields == null) {
			cachedFields = new int[tileCruxtruder.getFieldCount()];
			allFieldsHaveChanged = true;
		}
		for (int i = 0; i < cachedFields.length; ++i) {
			if (allFieldsHaveChanged || cachedFields[i] != tileCruxtruder.getField(i)) {
				cachedFields[i] = tileCruxtruder.getField(i);
		...
		for (IContainerListener listener : this.listeners) {
			for (int fieldID = 0; fieldID < tileCruxtruder.getFieldCount(); ++fieldID) {
	...
	public void updateProgressBar(int id, int data) {
		tileCruxtruder.setField(id, data);
	}
	...
	public class SlotOutput extends Slot {
		...
		public boolean isItemValid(ItemStack stack) {
			return TileCruxtruder.isItemValidForOutputSlot(stack);
		}
	}
}
