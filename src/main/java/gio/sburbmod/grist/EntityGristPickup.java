...
public class EntityGristPickup extends Entity
{
    ...
    public int gristValue;
    /** The closest EntityPlayer to this orb. */
    private EntityPlayer closestPlayer;
    /** Threshold color for tracking players */
    private int xpTargetColor;
    
    private int gristType;

    public EntityGristPickup(World worldIn, double x, double y, double z, int gristAmount, int gristType)
    {
        super(worldIn);
        this.setSize(0.5F, 0.5F);
        this.setPosition(x, y, z);
        this.rotationYaw = (float)(Math.random() * 360.0D);
        this.motionX = (double)((float)(Math.random() * 0.1 - 0.1) * 2.0F);
        this.motionY = (double)((float)(Math.random() * 0.2D) * 2.0F);
        this.motionZ = (double)((float)(Math.random() * 0.1 - 0.1) * 2.0F);
        this.gristValue = gristAmount;
        this.gristType = gristType;
        System.out.println("Summoned entity!");
    }
    ...
    public EntityGristPickup(World worldIn)
    {
        super(worldIn);
    ...
    public void writeEntityToNBT(NBTTagCompound compound)
    {
        ...
        compound.setShort("Type", (short)this.gristType);
    }
    ...
    public void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        this.gristType = compound.getShort("Type");
    }
    ...
    public void onCollideWithPlayer(EntityPlayer entityIn)
    {
        if (!this.world.isRemote)
        {
            if (this.delayBeforeCanPickup == 0 && entityIn.xpCooldown == 0)
            {
                ...
                {
                    entityIn.addExperience(this.gristValue);
    ...
    {
        if (this.gristValue >= 2477)
        {
            return 10;
        }
        else if (this.gristValue >= 1237)
        {
            return 9;
        }
        else if (this.gristValue >= 617)
        {
            return 8;
        }
        else if (this.gristValue >= 307)
        {
            return 7;
        }
        else if (this.gristValue >= 149)
        {
            return 6;
        }
        else if (this.gristValue >= 73)
        {
            return 5;
        }
        else if (this.gristValue >= 37)
        {
            return 4;
        }
        else if (this.gristValue >= 17)
        {
            return 3;
        }
        else if (this.gristValue >= 7)
        {
            return 2;
        }
        else
        {
            return this.gristValue >= 3 ? 1 : 0;
        }
    }
