...
public class GuiInventoryPrinter extends GuiContainer {

	// This is the resource location for the background image
	private static final ResourceLocation texture = new ResourceLocation("sburbmod", "textures/gui/cardprinter_bg.png");
	private TilePrinter tileEntity;

	public GuiInventoryPrinter(InventoryPlayer invPlayer, TilePrinter tileCruxtruder) {
		super(new ContainerPrinter(invPlayer, tileCruxtruder));
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int x, int y) {
		...
		int yOffset = (int) ((1 - cookProgress) * FLAME_HEIGHT);
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		final int INSTRUCTION_LABEL_XPOS = 7;
		final int INSTRUCTION_LABEL_YPOS = 114;
		TextComponentTranslation instr = new TextComponentTranslation("text.printer.instructions");
		fontRenderer.drawString(instr.getFormattedText(), INSTRUCTION_LABEL_XPOS, INSTRUCTION_LABEL_YPOS,
				Color.darkGray.getRGB());

		String bluh = String.valueOf(tileEntity.getRequiredTime());
		
		fontRenderer.drawString(bluh, INSTRUCTION_LABEL_XPOS, INSTRUCTION_LABEL_YPOS-18,
				Color.darkGray.getRGB());
		...
		if (isInRect(guiLeft + FLAME_XPOS, guiTop + FLAME_YPOS, FLAME_WIDTH, FLAME_HEIGHT, mouseX,
				mouseY)) {
			hoveringText.add("Material Remaining:");
			int cookPercentage = (int) (100 * ((float)tileEntity.fuelTime / (float)tileEntity.getRequiredTime()));
			hoveringText.add(cookPercentage + "%");
		}
